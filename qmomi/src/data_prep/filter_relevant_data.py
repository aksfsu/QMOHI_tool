...
import nltk
# nltk.download('gutenberg')  ### need to run once in the beginning
...
class relevant_content:
	...
	def relevant_content_words(self, keywords):
		...
		for phrase in keywords:
			...
			# phraseList = re.split(', |_|-|!|\+', phrase)

			# keeps track of the keyword index in the content and retrieve the surrounding words
			# c = nltk.ConcordanceIndex(self.content.tokens, key=lambda s: s.lower())
			...
			# intersects = set(offsets_norm[0]).union(*offsets_norm[1:])
...
def convert_words_sentences(words_content):
	...
	# sentences_array = sentences_array[1:-1]  # to remove 1st and last sentence in the array as it's not complete sentence
...
def find_relevant_content(input_dataframe, keywords, output_dir):
	...
	for index, row in input_dataframe.iterrows():
		...
		if (content != "No content"):
			...
			print("\n\n\n University and content written :\n ", university, content)

			try:  ### try writing content in the text file
				relevant_content_file = output_dir + "/relevant_content.txt"
				outFile = open(relevant_content_file, 'w')
				outFile.write(content)
				outFile.close()

				uni_object = relevant_content(university, relevant_content_file)  ## creating object per university

				words_content_list = uni_object.relevant_content_words(keywords)  ## words_content here is list of lists
				...
				for words_content in words_content_list:
					...
					final_relevant_content.append(sentences_content)  ##final cintent after removing duplicate sentences
			...
			if removed_unicode.isspace() or not removed_unicode:  ## Check if final content is only space

				print("\n\nOnly space!")

			else:  ### writing to dataframe
	...
	return output_dataframe, keywords   # returning space added keywords for metric calculation


# input_dataframe=
# keywords=
# output_dir=
# find_relevant_content(input_dataframe, keywords, output_dir)